/* =========================================================
   SISTEMA DE MUEBLERÍA IA - Versión Maestro Integrada (Final V6.1)
  ...
  try {
    ...
    if (targetProduct && !pideCatalogo) {
      if (pideSoloParte && targetProduct.tipo === "combo" && Array.isArray(targetProduct.items)) {
        ...
        if (pieceFound) {
          // V6.1: Price surcharge (+200) for piece extracted from combo
          ...
          const sheet = "DETALLE DE PIEZA INDIVIDUAL:\n" + (pieceFound.titulo || pieceFound.nombre).toUpperCase() + "\n" +
            ...
            priceStr;
          await sendMessageToGHL(contactId, sheet + "\n\nPermítame comunicarle con un asesor para confirmar disponibilidad. 😉", env, trace, pieceFound.url || pieceFound.link_publico || [], (env.GHL_LOCATION_ID || contact.locationId), conversationId);
          await triggerHandover(contactId, env, trace);
          return;
        }
      }
