/* =========================================================
   SISTEMA DE MUEBLERÍA IA - Versión Maestro Integrada (Final V6.0)
   - Fix: Spaced catalog (Visual improvements with double newlines)
   - Fix: Individual piece request (Provides specs + Immediate handover)
   - Fix: Structural/Complex query handover ([TRANSFERIR] for unanswerable topics)
   - Fix: Metadata leakage & Repetition control
...
function detectarSeleccionNatural(mensaje, lista) {
  ...
  if (nums) {
    for (let n of nums) {
      const pIdx = lista.findIndex(item => item.precio && item.precio.toString() === n);
...
async function callVendedorElitePro(message, contact, env, productoActual, intencionCierre, coverage, esSoloSaludo = false, esPrimerMensaje = false, yaEnvioMenu = false, esNuevoProducto = false, confirmandoCat = null) {
  ...
  let reglas = [
    ...
    "2. LISTAS: Viñetas ✨ (solo para componentes o características).",
    "3. NATURALIDAD: PROHIBIDO usar etiquetas como 'Producto:', 'Resumen:', 'Estado:', o '¡Invita a comprar!'. Escribe como un humano en un chat.",
    "4. PRESENTACIÓN: Si esNuevoProducto=TRUE, resume la descripción amablemente sin dar especificaciones técnicas.",
    "5. COMBOS: Datos detallados de CADA componente si piden.",
    "6. SOLO LO SOLICITADO.",
    "7. AYUDA: " + (mostrarMenu ? "Opcional: Menciona que puedes dar detalles de Medidas o Colores." : "NO ayuda."),
    "8. COMPRA: Si esNuevoProducto=TRUE, pide amablemente los datos (Nombre, DPI, etc.) al final.",
    "9. EMOJIS: Máximo uno (fuera de listas).",
    "10. CIERRE: No pidas datos si hay dudas.",
    "11. SALUDO: " + instruccionSaludo,
    "12. ESTRUCTURA: Si el cliente pregunta sobre temas estructurales (ej: 'se desarma', 'es colgante', 'se dobla') y la información NO ESTÁ en los DATOS PRODUCTO, DEBES responder exactamente '[TRANSFERIR]'."
  ];
  if (confirmandoCat) {
    reglas.push("13. CONFIRMACIÓN: Cliente mencionó '" + confirmandoCat + "'. Pregunta si desea ver esa categoría o seguir con " + (productoActual ? productoActual.titulo : "lo actual") + ".");
  }
...
async function moduloCatalogo(message, contact, env, trace, forcingCat = null) {
  ...
  // V6.0: Visual improvement with better spacing
  let resp = "Aquí tiene las opciones de " + cat.toUpperCase() + (tamano ? " " + tamano.toUpperCase() : "") + "S disponibles:\n\n";
  ...
  resp += "--------------------------\n";
  resp += "¿Cuál de estos le gustaría conocer a detalle? 😉";
  return {
    text: resp
  };
}
...
async function processFullFlow(rawMsg, contactId, contact, env, trace, conversationId = null) {
  try {
    ...
    const pideCompra = /\b(quiero comprar|lo quiero|la quiero|comprarlo|comprarla|pedido|ordenar|pagar)\b/i.test(norm);
    ...
    if (currentEstado === "esperando_departamento") {
      ...
      if (foundDept) {
        ...
        if (munProp) {
          ...
          if (!mun) {
            mun = await fuzzyMatchMunicipio(munProp, municipios, env, trace);
    ...
    if (targetProduct && tieneCategoria && !pideInformacion && !pideFotos) {
      if (!normalizarTextoGlobal(targetProduct.titulo).includes(catMencionada)) {
        ...
        const resp = await callVendedorElitePro(message, contact, env, targetProduct, false, null, esSoloSaludo, currentEstado === "nuevo", yaEnvioMenu, false, catMencionada);
    ...
    if (targetProduct && !pideCatalogo) {
      // V6.0: "Solo el" piece request handling
      if (pideSoloParte && targetProduct.tipo === "combo" && Array.isArray(targetProduct.items)) {
        const itemKeywords = ["ropero", "cama", "cabecera", "mesita", "cocina", "mueble", "gavetero", "tocador", "platera", "marquesa", "trinchante"];
        ...
        if (pieceFound) {
          const sheet = "DETALLE DE PIEZA INDIVIDUAL:\n" + (pieceFound.titulo || pieceFound.nombre).toUpperCase() + "\n" +
            "📏 Medidas: " + (pieceFound.medidas || "N/A") + "\n" +
            "🛠 Material: " + (pieceFound.estructura || "N/A") + "\n" +
            "🎨 Colores: " + (pieceFound.colores || "N/A");
          await sendMessageToGHL(contactId, sheet + "\n\nPermítame comunicarle con un asesor para confirmar disponibilidad y precio por separado. 😉", env, trace, pieceFound.url || pieceFound.link_publico || [], (env.GHL_LOCATION_ID || contact.locationId), conversationId);
          await triggerHandover(contactId, env, trace);
          return;
        }
      }
      ...
      // V6.0: Automatic handover if AI requests transfer
      if (resp && resp.includes("[TRANSFERIR]")) {
        const cleanedResp = resp.replace("[TRANSFERIR]", "").trim() || "Le pondré en contacto con un asesor para resolver sus dudas técnicas. 😉";
      ...
      let final = resp;
      if (!resp.toUpperCase().includes(targetProduct.titulo.toUpperCase())) final = "*" + targetProduct.titulo.toUpperCase() + "*\n\n" + resp;
      ...
      if (resp && resp.includes("[TRANSFERIR]")) {
        const cleanedResp = resp.replace("[TRANSFERIR]", "").trim() || "Un asesor le ayudará con su consulta en un momento. 😉";
        await sendMessageToGHL(contactId, cleanedResp, env, trace, [], (env.GHL_LOCATION_ID || contact.locationId), conversationId);
        await triggerHandover(contactId, env, trace);
        return;
      }
