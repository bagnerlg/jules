/* =========================================================
   SISTEMA DE MUEBLERÍA IA - Versión Maestro Integrada (Final V5.4)
...
async function processFullFlow(rawMsg, contactId, contact, env, trace, conversationId = null) {
  try {
    ...
    if (currentEstado === "esperando_departamento") { const depmunRaw = await env.COVERAGE_DB.get("listado:depmun"); const depmun = JSON.parse(depmunRaw || "{}"); const foundDept = Object.keys(depmun).find(d => norm.includes(d)); if (foundDept) { await setCustomFieldValue(contact, fDept, foundDept, env, trace); await setCustomFieldValue(contact, fEstado, "esperando_municipio", env, trace); await sendMessageToGHL(contactId, "¿En qué municipio de " + foundDept.toUpperCase() + " está?", env, trace, [], null, conversationId); return; } await sendMessageToGHL(contactId, "¿En qué departamento de Guatemala se encuentra? 🇬🇹", env, trace, [], null, conversationId); return; }
    ...
    if (pideCobertura && !pideInformacion) { const resp = await obtenerRespuestaCoverage(message, env, trace); if (resp) { await sendMessageToGHL(contactId, resp, env, trace, [], null, conversationId); return; } await setCustomFieldValue(contact, fEstado, "esperando_departamento", env, trace); await sendMessageToGHL(contactId, "Con gusto. ¿En qué departamento se encuentra? 📍", env, trace, [], null, conversationId); return; }
