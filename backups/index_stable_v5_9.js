/* =========================================================
   SISTEMA DE MUEBLERÍA IA - Versión Maestro Integrada (Final V5.9)
   - Fix: Metadata leakage (Forbids structured labels in AI output)
   - Fix: Auto-selection for single-item catalogs
   - Fix: Robust gender/plural size support (mediana, medianos, etc.)
   - Fix: Expanded size synonyms (Supports "estandar", "media", "grando", etc.)
   - Fix: Price-based selection in catalog (Allows picking by "Q3499", "3500", etc.)
...
function detectarSeleccionNatural(mensaje, lista) {
  ...
  // V5.9: Auto-selection for single-item catalogs if interest is shown
  if (lista.length === 1) {
...
async function callVendedorElitePro(message, contact, env, productoActual, intencionCierre, coverage, esSoloSaludo = false, esPrimerMensaje = false, yaEnvioMenu = false, esNuevoProducto = false, confirmandoCat = null) {
  ...
  // V5.9: Strict rules to prevent metadata leakage and structural repetition
  let reglas = [
  ...
  if (confirmandoCat) {
    reglas.push("12. CONFIRMACIÓN: Cliente mencionó '" + confirmandoCat + "'. Pregunta si desea ver esa categoría o seguir con " + (productoActual ? productoActual.titulo : "lo actual") + ".");
  }
  ...
  try {
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "system",
          content: "Asesor de chat. Habla natural. Sin etiquetas estructuradas."
        }, {
          role: "user",
          content: prompt
        }],
...
async function moduloCatalogo(message, contact, env, trace, forcingCat = null) {
  ...
  resultados.forEach((p, i) => {
    resp += (i + 1) + ". " + (p.nombre || p.titulo).toUpperCase() + " - Q" + p.precio + "\n";
  });
